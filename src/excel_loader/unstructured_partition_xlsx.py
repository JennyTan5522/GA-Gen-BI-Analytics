...
class _XlsxPartitionerOptions:
    ...
    def expand_merged_cells(self, sheet_name) -> pd.DataFrame:
        """Create a DataFrame where merged cells are expanded while retaining both values and cell addresses."""
        wb = load_workbook(self._file_path, data_only=True)
        sheet = wb[sheet_name]

        # Initialize data list to store (address, value) tuples
        data = []
        for row in sheet.iter_rows():
            row_data = []
            for cell in row:
                value = cell.value if cell.value is not None else np.nan
                row_data.append(value if value is not np.nan else np.nan)
                # row_data.append({cell.coordinate: value} if value is not np.nan else np.nan)  # Store (address, value) or np.nan
            data.append(row_data)
    
        # Create a deep copy for expansion
        expanded_data = [[cell for cell in row] for row in data]
    
        # Expand merged cells
        for merged_range in sheet.merged_cells.ranges:
            min_col, min_row, max_col, max_row = merged_range.bounds
            top_left_cell = sheet.cell(row=min_row, column=min_col)
            top_left_value = top_left_cell.value if top_left_cell.value is not None else np.nan
            top_left_address = top_left_cell.coordinate
    
            # Fill merged cells with the top-left value (but not address)
            for row in range(min_row - 1, max_row):  # Convert 1-based to 0-based index
                for col in range(min_col - 1, max_col):  # Convert 1-based to 0-based index
                    expanded_data[row][col] = top_left_value if top_left_value is not np.nan else np.nan
                    # expanded_data[row][col] = {top_left_address: top_left_value} if top_left_value is not np.nan else np.nan
    ...
    def sheets(self) -> dict[str, pd.DataFrame]:
        ...
        if file_path := self._file_path:
            """The spreadsheet worksheets, each as a data-frame mapped by sheet-name, but with merged cells expanded."""
            wb = load_workbook(file_path, data_only=True)
            return {sheet.title: self.expand_merged_cells(sheet_name=sheet.title) for sheet in wb.worksheets}
            # return {sheet.title: self.expand_merged_cells(sheet_name=sheet.title) for sheet in wb.worksheets}
...
class _ConnectedComponent:
    ...
    def _init_(self, worksheet: pd.DataFrame, cell_coordinate_set: set[_CellCoordinate]):
...
class _ConnectedComponents:
    ...
    def _init_(self, worksheet_df: pd.DataFrame):
        self._worksheet_df = worksheet_df

    def _iter_(self) -> Iterator[_ConnectedComponent]:
...
class _SubtableParser:
    ...
    def _init_(self, subtable: pd.DataFrame):
