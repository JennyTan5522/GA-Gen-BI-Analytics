...
class UIManager:
    ...
    def react_agent_toolkit(self, retriever_top_k_documents: str, additional_feedbacks: Optional[str] = None):
        ...
        tools.extend([FollowUpQuestionTool(llm=st.session_state.llm),])
    ...
    def fix_final_answer(self, original_response: str, error_msg: str):
        ...
        try:
            ...
            chain = fixed_prompt_template | self.app.llm | StrOutputParser()
    ...
    def invoke_agent_response(self, agent_executor: AgentExecutor, user_query: str, user_feedback = False):
        ...
        return saved_user_message, response_text
    ...
    def final_response_output_parser(self, response_text: str):
        ...
        return sql, text_response, code_block, final_answer_str_format
    ...
    def display_feedback_form(self, user_query: str, sql_query: str):
        """
        ...
        with st.form("feedback_form"):
            streamlit_feedback(feedback_type="thumbs", optional_text_label="[Optional] 👍 If you want to save this response for feedback reference, click thumbs up. 👎 If you want to regenerate or provide corrections, click thumbs down and explain below.", align="flex-start", key='feedback')
            st.form_submit_button('Submit Response', on_click=lambda: self.handle_user_feedback(user_query, sql_query)) 
    ...
    def handle_user_feedback(self, user_query: str, sql_query: str):
        """
        ...
        Args:
            user_query (str): The original query provided by the user.
            sql_query (str): The SQL query generated by the LLM in the last response.
        """
        ...
        if hasattr(last_message, "content") and last_message.content:
            ...
            if feedback_score == "👎":
                ...
                user_query, response_output = self.invoke_agent_response(agent_executor, user_query, user_feedback = True)
                sql, text_response, code_block, final_answer_str_format = self.final_response_output_parser(response_output["output"])
    ...
    def display_response(self, sql: str, response: str, code_block: str, agent_thoughts: str = None):
        ...
        try:
            ...
            if sql_is_blank and code_is_blank:
                ...
                chart_tab, sql_tab, code_tab, agent_tab = st.tabs(["📊 Chart", "🛢 SQL", "💻 Code", "🧠 Agent Thinking"])
                ...
                sql_tab, agent_tab = st.tabs(["🛢 SQL", "🧠 Agent Thinking"])
                    ...
                    with st.spinner("We are preparing a response to your question. Please allow up to one minute for completion...."):
                        ...
                        while attempt < MAX_RETRIES:
                            try:
                                ...
                                retriever_top_k_documents = self.rag_top_k_related_documents(user_query)
                                print("Retrieved top-k: ", retriever_top_k_documents)
                                ...
                                sql, text_response, code_block, final_answer_str_format = self.final_response_output_parser(response_output["output"])
                                self.display_response(sql, text_response, code_block, response_output["intermediate_steps"])
                                self.saved_user_message(user_query, final_answer_str_format, response_output["intermediate_steps"])

                                # Show feedback form for user to provide feedback on the response
                                self.display_feedback_form(user_query, sql)
                                break
                            except Exception as e:
                                try:
                                    ...
                                    sql, text_response, code_block, final_answer_str_format = self.final_response_output_parser(fix_output)
                                    self.display_response(sql, text_response, code_block, response_output["intermediate_steps"])
                                    self.saved_user_message(user_query, final_answer_str_format, response_output["intermediate_steps"])
                                    break
                                except Exception as e:
